...
class MockData {
  ...
  static List<CardItem> _albumCards(
    int from,
    int count, {
    required String sub,
  }) {
    return List<CardItem>.generate(count, (i) {
      ...
      return CardItem(
        id: 'a${t.id}',
        title: t.title,
        subtitle: sub.isEmpty ? t.artist : sub,
  ...
  static List<Shelf> get shelves => <Shelf>[
    Shelf(
      title: '为你推荐',
      items: _albumCards(0, 6, sub: ''),
    ),
    Shelf(
      title: '热门音乐',
      items: _albumCards(3, 6, sub: ''),
    ),
    const Shelf(title: '关注的 UP 主', items: _creators),
    Shelf(
      title: '原创音乐排行榜',
      items: _albumCards(6, 6, sub: ''),
    ),
  ];
  ...
  static List<({Track track, double progress, bool done})> get downloads =>
      <({Track track, double progress, bool done})>[
